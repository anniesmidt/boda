...
class WDSControllerSliders_wds {
  ...
  public function import() {
    ...
    $flag = FALSE;
    $file = isset($_FILES['fileimport']) ? $_FILES['fileimport'] : null;
    if ($file) {
      $zip = zip_open($file["tmp_name"]);  
      if ($zip) {
        $dest_dir = ABSPATH . $WD_S_UPLOAD_DIR;
        if (!is_dir($dest_dir . '/import')) {
          mkdir($dest_dir . '/import', 0777);
        }
        if (!is_dir($dest_dir . '/import/arrows')) {
          mkdir($dest_dir . '/import/arrows', 0777);
        }
        if (!is_dir($dest_dir . '/import/arrows/thumb')) {
          mkdir($dest_dir . '/import/arrows/thumb', 0777);
        }
        if (!is_dir($dest_dir . '/import/arrows/.original')) {
          mkdir($dest_dir . '/import/arrows/.original', 0777);
        }
        if (!is_dir($dest_dir . '/import/.original')) {
          mkdir($dest_dir . '/import/.original', 0777);
        }
        if (!is_dir($dest_dir . '/import/thumb')) {
          mkdir($dest_dir . '/import/thumb', 0777);
        }
        ...
        while ($zip_entry = zip_read($zip)) {
          if (strripos(zip_entry_name($zip_entry), ".xml")) {
              ...
              foreach ($sliders as $slider) {
                foreach ($slider as $key_slider => $value_slider) {
                  ...
                  if (strpos($value_slider["value"], FAKE_SITE_URL_BTNS) === 0) {
                    $slider_fields[$key_slider] = str_replace(FAKE_SITE_URL_BTNS, $dest_url . '/import/arrows/', $value_slider["value"]);
                    }
                    elseif (strpos($value_slider["value"], FAKE_SITE_URL) === 0) {
                      $slider_fields[$key_slider] = str_replace(FAKE_SITE_URL, site_url(), $value_slider["value"]);
                  }
                  elseif ($key_slider != "slide" && $key_slider != "id") {
                    $slider_fields[$key_slider] = $value_slider["value"];
                  }
                  elseif ($key_slider == "slide") {
                    foreach ($value_slider->children() as $key_slide => $slide) {
                      if ($key_slide != "layer" && $key_slide != "id") {
                        $slide_fields[$key_slide] = $slide["value"];
                      }
                      elseif ($key_slide == "layer") {
                        foreach ($slide->children() as $key_layer => $layer) {
                          if ($key_layer != "id") {
                            $layer_fields[$key_layer] = $layer["value"];
                          }
                        }
                ...
                foreach ($slides as $slide) {
                  ...
                  if (strpos($slide["slide"]["image_url"], FAKE_SITE_URL) === 0) {
                    $slide["slide"]["image_url"] = str_replace(FAKE_SITE_URL, $dest_url . '/import/', $slide["slide"]["image_url"]);
                  }
                  if (strpos($slide["slide"]["thumb_url"], FAKE_SITE_URL) === 0) {
                    $slide["slide"]["thumb_url"] = str_replace(FAKE_SITE_URL, $dest_url . '/import/thumb/', $slide["slide"]["thumb_url"]);
                  }
                  ...
                  foreach ($slide["layers"] as $layer) {
                    ...
                    if (strpos($layer["image_url"], FAKE_SITE_URL) === 0) {
                      $layer["image_url"] = str_replace(FAKE_SITE_URL, $dest_url . '/import/', $layer["image_url"]);
                    }
            ...
            if (strpos($zip_name, 'thumb_') === 0) {
              ...
              $zip_name = $this->get_unique_file_name($zip_name,$dest_dir . '/import/thumb/', $zip_name);
            ...
            elseif (strpos($zip_name, FAKE_SITE_URL_BTNS . '_thumb_') === 0) {
              ...
              $zip_name = $this->get_unique_file_name($zip_name,$dest_dir . '/import/arrows/thumb/', $zip_name);
            ...
            elseif (strpos($zip_name, FAKE_SITE_URL_BTNS) === 0) {
              ...
              $zip_name = $this->get_unique_file_name($zip_name, $dest_dir . '/import/arrows/.original/', $zip_name);
              if ($handleorg = fopen($dest_dir . '/import/arrows/.original/' . $zip_name, "w")) {
                fwrite($handleorg, $zip_r);
                fclose($handleorg);
              } 
              $zip_name = $this->get_unique_file_name($zip_name,$dest_dir . '/import/arrows/', $zip_name);
              ...
              $zip_name = $this->get_unique_file_name($zip_name , $dest_dir . '/import/.original/',$zip_name);
              if ($handleorg = fopen($dest_dir . '/import/.original/' . $zip_name, "w")) {
                fwrite($handleorg, $zip_r);
                fclose($handleorg);
              }
              $zip_name = $this->get_unique_file_name($zip_name , $dest_dir . '/import/', $zip_name);
    ...
    if ($flag) {
      echo WDW_S_Library::message("Slider successfully imported.", 'updated');
    }
    else {
      echo WDW_S_Library::message("Unexpected error occurred.", 'error');
    }
    $this->display();
  }

  public function get_unique_file_name($filename, $foldername, $zip_name) {
  ...
  public function save_slider_db() {
    ...
    $rl_butt_style = ((isset($_POST['rl_butt_style'])) ? esc_html(stripslashes($_POST['rl_butt_style'])) : 'fa-angle');
    ...
    $butts_transparent = ((isset($_POST['butts_transparent'])) ? (int) esc_html(stripslashes($_POST['butts_transparent'])) : 100);
    ...
    $bull_style = ((isset($_POST['bull_style'])) ? esc_html(stripslashes($_POST['bull_style'])) : 'fa-square-o');
    $bull_size = ((isset($_POST['bull_size'])) ? (int) esc_html(stripslashes($_POST['bull_size'])) : 20);
    ...
    $carousel = ((isset($_POST['carousel'])) ? (int) esc_html(stripslashes($_POST['carousel'])) : 0);
    $carousel_image_counts = ((isset($_POST['carousel_image_counts'])) ? (int) esc_html(stripslashes($_POST['carousel_image_counts'])) : 7);
    $carousel_image_parameters = ((isset($_POST['carousel_image_parameters'])) ?  esc_html(stripslashes($_POST['carousel_image_parameters'])) : 0.85);
    $carousel_fit_containerWidth = ((isset($_POST['carousel_fit_containerWidth'])) ? (int) esc_html(stripslashes($_POST['carousel_fit_containerWidth'])) : 0);
    $carousel_width = ((isset($_POST['carousel_width'])) ? (int) esc_html(stripslashes($_POST['carousel_width'])) : 1000);
