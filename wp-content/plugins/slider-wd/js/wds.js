...
function wds_add_post(ids_string, single) {
  ...
  for (var i in ids_array) {
    if (ids_array.hasOwnProperty(i) && ids_array[i]) {
      var id = ids_array[i];
      if (jQuery("#check_" + id).attr('checked') == 'checked' || single) {
        if (!single) {
...
function spider_get_video_info(input_id, id, type, prefix) {
    ...
    case 'YOUTUBE':
      ...
      if (video_id) {
        jQuery.getJSON('http://gdata.youtube.com/feeds/api/videos/'+video_id+'?v=2&alt=jsonc',function(data,status,xhr){
          ...
          fileData['thumb_url'] = "https://i.ytimg.com/vi/" + video_id + "/hqdefault.jpg";
          fileData['thumb'] = "https://i.ytimg.com/vi/" + video_id + "/hqdefault.jpg";
          ...
          if (type == 'layer') {
            jQuery("#" + input_id).val("");
            jQuery("#" + prefix).attr("src", fileData['thumb_url']);
            jQuery("#" + prefix + "_image_url").val(fileData['thumb_url']);
            jQuery("#" + prefix + "_link").val(fileData['filename']);
            jQuery("#" + prefix + "_alt").val(host);
...
function wds_add_video_layer(id) {
  jQuery('#add_video_button').attr("onclick", "wds_add_layer('video', '" + id + "');  return false;");
...
function wds_add_layer(type, id, layerID, duplicate, files, edit) {
  ...
  var text_dimensions = '<td class="spider_label"><label for="' + prefix + '_image_width">Dimensions: </label></td>' +
	...
	if (spider_uploader) {
		update_img_layer_upload_by = ' <a href="' + uploader_href_for_add_layer + '" class="button-primary button button-small thickbox thickbox-preview" title="Edit Image" onclick="return false;">Edit Image</a>';
  }
  else {
		update_img_layer_upload_by = ' <input class="button-primary button button-small" type="button" value="Edit Image" onclick="wds_add_layer(\'image\', \'' + id + '\', \'' + layerID + '\', \'\', \'\', 1); return false;" />';
  }
  ...
  var dimensions = '<td class="spider_label"><label>Dimensions: </label></td>' +
                   '<td>' +
                     ...
                     '<input type="checkbox" name="' + prefix + '_image_scale" id="' + prefix + '_image_scale" onchange="wds_scale(this, \'' + prefix + '\')" /><label for="' + prefix + '_image_scale">Scale</label>' +
                      update_img_layer_upload_by +
                     '<div class="spider_description">Set width and height of the image.</div></td>';
  var video_dimensions = '<td class="spider_label"><label>Dimensions: </label></td>' +
                   '<td>' +
                     '<input type="hidden" name="' + prefix + '_image_url" id="' + prefix + '_image_url" />' +
                     '<input type="text" name="' + prefix + '_image_width" id="' + prefix + '_image_width" value="" class="spider_int_input" onkeyup="wds_scale(\'#' + prefix + '_image_scale\', \'' + prefix + '\')" /> x ' +
                     '<input type="text" name="' + prefix + '_image_height" id="' + prefix + '_image_height" value="" class="spider_int_input" onkeyup="wds_scale(\'#' + prefix + '_image_scale\', \'' + prefix + '\')" /> px ' +
                     '<div class="spider_description">Set width and height of the video.</div></td>';
  ...
  var layer_effect_out = '<td class="spider_label"><label>Effect out: </label></td>' +
                   '<td>' +
                    ...
                    '</span>' +
                   '</td>';
  var video_url = '<input type="hidden" name="' + prefix + '_link" id="' + prefix + '_link" value="" />';
  var video_host = '<input type="hidden" name="' + prefix + '_alt" id="' + prefix + '_alt" value="" />';
  var video_thumb = '<input type="hidden" name="' + prefix + '_image_url" id="' + prefix + '_image_url" value="" />';
  var video_autoplay = '<td class="spider_label"><label>Autoplay: </label></td>' +
                  '<td><input type="radio" id="' + prefix + '_image_scale1" name="' + prefix + '_image_scale" checked="checked" value="on" ><label for="' + prefix + '_image_scale1">Yes</label>' +
                      '<input type="radio" id="' + prefix + '_image_scale0" name="' + prefix + '_image_scale" value="off" /><label for="' + prefix + '_image_scale0">No</label><div class="spider_description"></div></td>';
  ...
  switch(type) {
    ...
    case 'video': {
      ...
      jQuery("#" + tbodyID).append(tbody +
        '<tr class="wds_layer_tr">' +
          video_url + video_host + video_thumb +
          video_dimensions +
          layer_effect_in +
        '</tr><tr class="wds_layer_tr">' +
          position +
	  ...
	  if(!duplicate) {
        if (spider_get_video_info('video_url', id, 'layer', prefix)) {
...
function wds_add_slide() {
  ...
  if (spider_uploader) {
    ...
    img_layer_upload_by = ' <a href="' + uploader_href_for_add_layer + '" class="button-primary button button-small thickbox thickbox-preview" title="Add Image Layer" onclick="return false;">Add Image Layer</a>';
  }
  else {
    ...
    img_layer_upload_by = ' <input class="button-primary button button-small" type="button" value="Add Image Layer" onclick="wds_add_layer(\'image\', \'' + slideID + '\', \'\'); return false;" />';
  }
      ...
      '<table class="ui-sortable' + slideID + '">' +
        ...
        '<tbody>' +
          ...
          '<tr><td colspan="4">' +
            ...
            ' <a href="' + wds_add_post_href.replace("action=WDSPosts", "action=WDSPosts&slide_id=" + slideID) + '" class="button-secondary thickbox thickbox-preview" title="Add Post" onclick="if (wds_check_required(\'name\', \'Name\')) {jQuery(this).removeClass(\'thickbox\').removeClass(\'thickbox-preview\');return false;}; jQuery(this).addClass(\'thickbox\').addClass(\'thickbox-preview\'); return false;">Add Post</a>' +
          ...
          '</tr><tr><td colspan="4">' +
            ...
            img_layer_upload_by +
            ' <input type="button" class="button-primary button button-small" onclick="wds_add_video_layer(\'' + slideID + '\'); return false;" value="Add Video Layer" />' +
